...
@WebServlet(name = "TableOrderController", urlPatterns = {"/staff/table-order"})
public class TableOrderController extends HttpServlet {
    ...
    /**
     * 🧾 Hiển thị lịch sử order của bàn + danh sách món
     */
    private void showOrderHistory(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            e.printStackTrace();
            response.sendRedirect("table-order?action=list");
        }
    }

    /**
     * ➕ Staff thêm món vào order của bàn
     */
    private void addItem(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean added = false;
            if (orderId > 0) {
                added = dao.addItemToOrder(orderId, menuItemId, quantity);
            }
            ...
            response.sendRedirect("table-order?action=orderHistory&id=" + tableId);
...
private void updateStatus(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        if ("SEATED".equalsIgnoreCase(status)) {
        ...
        response.sendRedirect(request.getContextPath() + "/staff/table-order?action=list");
