...
// 🔵 Tạo hoặc lấy order đang hoạt động cho bàn (staff - khách vãng lai)
    ...
    public Map<String, Object> getInvoiceHeaderByReservationId(long reservationId) {
        String sql = """
        SELECT 
            r.reservation_id,
            r.customer_id,
            ...
            r.status,
            c.full_name AS customer_name,
            STRING_AGG(t.code, ', ') WITHIN GROUP (ORDER BY t.code) AS table_codes,
            DATEADD(MINUTE, r.reserved_duration, r.reserved_at) AS end_at
        FROM Reservation r
        LEFT JOIN Customer c      ON c.customer_id = r.customer_id
        LEFT JOIN BookingTable bt ON bt.reservation_id = r.reservation_id
        LEFT JOIN RestaurantTable t ON t.table_id = bt.table_id
        WHERE r.reservation_id = ?
        GROUP BY r.reservation_id, r.customer_id, r.reserved_at, r.reserved_duration, r.status, c.full_name
    """;
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    m.put("reservation_id", rs.getLong("reservation_id"));
                    m.put("customer_id", rs.getString("customer_id"));
                    m.put("customer_name", rs.getString("customer_name"));
                    m.put("reserved_at", rs.getTimestamp("reserved_at"));
                    m.put("end_at", rs.getTimestamp("end_at"));
                    m.put("status", rs.getString("status"));
                    m.put("table_codes", rs.getString("table_codes")); // có thể null nếu chưa gán bàn
                    return m;
                }
            }
        } catch (SQLException e) {
            System.out.println("[OrderDAO] getInvoiceHeaderByReservationId() failed: " + e.getMessage());
        }
        return null;
    }

    public List<Map<String, Object>> getInvoiceLinesByReservationId(long reservationId) {
        // Lấy mọi Order thuộc reservation, rồi join items + menu name
        String sql = """
        SELECT 
            m.name AS item_name,
            oi.quantity,
            oi.price,
            CAST(oi.quantity * oi.price AS DECIMAL(18,2)) AS line_total
        FROM Orders o
        JOIN OrderItems oi ON oi.order_id = o.order_id
        JOIN MenuItem m    ON m.id = oi.menu_item_id
        WHERE o.reservation_id = ?
        ORDER BY oi.created_at ASC, m.name
    """;
        List<Map<String, Object>> list = new ArrayList<>();
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setLong(1, reservationId);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    row.put("subtotal", rs.getDouble("line_total"));
                    list.add(row);
                }
            }
        } catch (SQLException e) {
            System.out.println("[OrderDAO] getInvoiceLinesByReservationId() failed: " + e.getMessage());
        }
        return list;
    }

    public List<Map<String, Object>> getOrdersByReservationId(long reservationId) {
        String sql = """
        SELECT *
        FROM Orders 
    ...
    """;
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            System.out.println("[OrderDAO] getOrdersByReservationId() failed: " + e.getMessage());
        }
        return list;
    }

    public long addOrder(long reservationId, int tableId, String customerId, String note) {
         long newOrderId = -1;
        String sql = """
            INSERT [dbo].[Orders] ([reservation_id], [table_id], [created_at], [amount], [status], [order_type], [note], [customer_id]) 
            OUTPUT INSERTED.order_id
            VALUES (?, ?, SYSDATETIME(), 0, N'PENDING', N'DINE_IN', ?, ?)
        """;
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setLong(1, reservationId);
            ps.setInt(2, tableId);
     ...
     public boolean addItemToOrder(long orderId, int menuItemId, int quantity, double price) {
        String sql = """
            INSERT INTO OrderItems (order_id, menu_item_id, quantity, price, created_at)
            VALUES (?, ?, ?, ?, SYSDATETIME())       
        """;
     ...
     public boolean updateOrderAmount(long orderId, double amount) {
