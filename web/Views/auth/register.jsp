...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const passNew = document.querySelector("input[name='newPassword']");
  const passConfirm = document.querySelector("input[name='confirmPassword']");
  const form = document.querySelector("form");

  // Regex cho tiếng Việt: chỉ chữ cái và khoảng trắng
  const nameRegex = /^[A-Za-zÀ-Ỹà-ỹĐđ\s]{2,50}$/;

  function validateName(input) {
    if (!input) return true;
    const ok = nameRegex.test(input.value.trim());
    input.style.borderColor = ok ? "" : "red";
    return ok;
  }

  function validatePassword() {
    if (!passNew.value) return true;
    const ok = passNew.value.length >= 6 && passNew.value.length <= 20;
    passNew.style.borderColor = ok ? "" : "red";
    return ok;
  }

  function validateConfirm() {
    const ok = passConfirm.value === passNew.value;
    passConfirm.style.borderColor = ok ? "" : "red";
    return ok;
  }

  [firstName, lastName].forEach(i => i?.addEventListener("input", () => validateName(i)));
  passNew?.addEventListener("input", validatePassword);
